...
class RobotArmSimulator:
    ...
    def __init__(self, node, robot_id=10):
        ...
        # Trail buffer (deque with maxlen = O(1) append/evict, vs O(n) list.pop(0))
        ...
        # Redraw throttle: physics/publish stay at full tick rate, but the
        # actual (expensive) figure redraw only happens every N ticks.
        self.render_tick_counter = 0
    ...
    def update_arm(self):
        ...
        # Trail (deque auto-evicts oldest point once maxlen is reached)
    ...
    def update_plot(self):
        """Called from main timer to refresh the arm window. The actual
        (expensive) redraw only happens every RENDER_EVERY_N_TICKS ticks --
        state updates elsewhere still happen every tick."""
        self.render_tick_counter += 1
        if self.render_tick_counter % self.RENDER_EVERY_N_TICKS == 0:
            self.fig.canvas.draw_idle()
    ...
    def execute_callback(self, goal_handle):
        ...
        if self.arm_sim is not None and self.robot_id == self.arm_sim.robot_id:
            if target_state == 1:   # open
                ...
                self.node.release_stick(self.robot_id)
            elif target_state == 2: # close
                self.arm_sim.gripper_opening = 0.0
                self.node.try_grab_stick(self.robot_id)
            else:
...
class MultiRoboMasterSim(Node):
    def __init__(self):
        ...
        # ------------------------------------------------------------------
        # STICK PICKUP / PUCK PUSH PHYSICS (kinematic, not a full physics engine)
        # ------------------------------------------------------------------
        # Approximate world position of a robot's gripper/end-effector: a fixed
        # distance forward of the robot's center, along its heading. This is
        # only an approximation -- the arm simulator's 2-link IK lives in its
        # own separate small-scale figure and isn't geometrically tied to the
        # robot's world pose, so we can't read an exact world-frame EE position.
        self.GRIPPER_REACH = 0.30          # meters forward of robot center
        self.GRAB_RANGE = 0.12             # how close the stick must be to grab it

        # Puck/stick are scaled up ("one size bigger") by this factor vs. the
        # original dimensions; collision constants below scale with them so
        # contact detection matches what's actually drawn.
        self.STICK_PUCK_SCALE = 1.4
        self.STICK_BLADE_LOCAL = np.array([0.18, 0.0]) * self.STICK_PUCK_SCALE  # blade contact point, stick's local frame
        self.PUCK_RADIUS = 0.025 * self.STICK_PUCK_SCALE
        self.STICK_BLADE_HALFWIDTH = 0.02 * self.STICK_PUCK_SCALE
        self.CONTACT_DIST = self.PUCK_RADIUS + self.STICK_BLADE_HALFWIDTH
        ...
        self.hockey_objects['sticks_1'] = {
            ...
            'patches': [],
            'held_by': None,       # robot_id currently gripping the stick, or None
            'grab_offset': None,   # (dx, dy, dtheta) of stick relative to holder's body frame
        }
        ...
        # Tracks the stick blade's world position tick-to-tick, so the puck
        # push (in _apply_hockey_physics) can be computed from the blade's
        # actual motion rather than snapping to a fixed distance every frame.
        self._prev_blade_pos = self._stick_blade_world_pos()
    ...
    def __init_plot(self):
        ...
        puck_circle = patches.Circle((puck_data['state'][0], puck_data['state'][1]),
                                     self.PUCK_RADIUS, facecolor='green', edgecolor='darkgreen', linewidth=1)
        ...
        stick_dot2 = patches.Circle((stick_data['state'][0], stick_data['state'][1]),
                                    0.015 * s, facecolor='orange', alpha=0.6)
        ...
        # Redraw throttle: physics/publish stay at full tick rate, but the
        # actual (expensive) figure redraw only happens every N ticks. This
        # replaces an earlier blitting approach that turned out to silently
        # hide all dynamic artists (robots/puck/stick) if the very first
        # background capture ran before the window was fully laid out by
        # the window manager -- a real risk over remote/forwarded X11.
        self.render_tick_counter = 0
        self.RENDER_EVERY_N_TICKS = 2
        ...
        for obj_key, obj_data in self.hockey_objects.items():
            ...
            for patch in obj_data['patches']:
                if patch.contains(event)[0]:
                    ...
                    # Manually grabbing the stick with the mouse overrides/drops
                    # any robot's grip on it, so dragging always wins.
                    if obj_key == 'sticks_1' and obj_data['held_by'] is not None:
                        self.release_stick(obj_data['held_by'])
    ...
    def on_motion(self, event):
        ...
        self.__update_plot()
        self.figure.canvas.draw_idle()  # force immediate redraw while dragging, bypassing the throttle
    ...
    # ------------------------------------------------------------------
    # STICK PICKUP / PUCK PUSH PHYSICS
    # ------------------------------------------------------------------
    def _gripper_world_pos(self, robot_id):
        """Approximate world position of robot_id's gripper (see GRIPPER_REACH note)."""
        state = self.states[robot_id]
        theta = state[2]
        return np.array([state[0] + self.GRIPPER_REACH * cos(theta),
                          state[1] + self.GRIPPER_REACH * sin(theta)])

    def try_grab_stick(self, robot_id):
        """Called when a gripper closes. Attaches the stick to the robot if
        it's within grab range. From then on the stick's orientation always
        exactly matches the robot's (no independent rotation), and it
        translates rigidly with the robot at a fixed forward offset -- as if
        physically clamped in the gripper."""
        stick = self.hockey_objects['sticks_1']
        if stick['held_by'] is not None:
            return  # already held (by this or another robot)
        gripper_pos = self._gripper_world_pos(robot_id)
        if np.linalg.norm(stick['state'][:2] - gripper_pos) <= self.GRAB_RANGE:
            # Fixed mount offset: directly in front of the robot, centered
            # (no sideways/rotational offset) -- orientation locks to the
            # robot's exactly (rel_theta = 0), matching it always.
            stick['held_by'] = robot_id
            stick['grab_offset'] = np.array([self.GRIPPER_REACH, 0.0, 0.0])
            self.get_logger().info(f"Robot {robot_id} grabbed the stick")

    def release_stick(self, robot_id):
        """Called when a gripper opens. Releases the stick if this robot was
        holding it, leaving it wherever it currently is."""
        stick = self.hockey_objects['sticks_1']
        if stick['held_by'] == robot_id:
            stick['held_by'] = None
            stick['grab_offset'] = None
            self.get_logger().info(f"Robot {robot_id} released the stick")

    def _stick_blade_world_pos(self):
        stick = self.hockey_objects['sticks_1']
        theta = stick['state'][2]
        R = np.array([[cos(theta), -sin(theta)], [sin(theta), cos(theta)]])
        return stick['state'][:2] + R @ self.STICK_BLADE_LOCAL

    def _apply_hockey_physics(self):
        """Runs once per tick, before hockey object poses are published:
        (1) if the stick is held, make it follow the holder robot's pose
            and orientation exactly;
        (2) if the stick's blade contacts the puck, push the puck smoothly
            in whatever direction the blade is actually moving into it --
            only the push (contact-normal) component transfers, so sweeping
            the stick sideways past the puck doesn't drag it along."""
        stick = self.hockey_objects['sticks_1']
        if stick['held_by'] is not None:
            holder = stick['held_by']
            theta = self.states[holder][2]
            R = np.array([[cos(theta), -sin(theta)], [sin(theta), cos(theta)]])
            offset = stick['grab_offset']
            world_xy = self.states[holder][:2] + R @ offset[:2]
            stick['state'][0] = world_xy[0]
            stick['state'][1] = world_xy[1]
            stick['state'][2] = theta + offset[2]  # offset[2] is always 0 -> orientation == robot's

        blade_pos = self._stick_blade_world_pos()
        blade_delta = blade_pos - self._prev_blade_pos  # how far the blade moved this tick

        # Stick pushes the puck on contact (skip if the person is dragging the puck)
        if not (self.dragging and self.dragged_object == 'puck_green'):
            puck = self.hockey_objects['puck_green']
            diff = puck['state'][:2] - blade_pos
            dist = np.linalg.norm(diff)
            if dist < self.CONTACT_DIST:
                normal = diff / dist if dist > 1e-6 else np.array([1.0, 0.0])
                # Only the part of the blade's motion pushing INTO the puck
                # (along -normal) transfers. A sideways/tangential sweep of
                # the blade contributes nothing here, so the puck doesn't
                # glide sideways with it -- only genuine forward contact
                # moves the puck, by exactly the same amount the stick moved.
                push_amount = -np.dot(blade_delta, normal)
                if push_amount > 0:
                    puck['state'][0] += normal[0] * push_amount
                    puck['state'][1] += normal[1] * push_amount
                # Safety clamp against any residual overlap (e.g. a very
                # fast tick) -- keeps contact exact without re-snapping the
                # puck's whole position every frame.
                diff2 = puck['state'][:2] - blade_pos
                dist2 = np.linalg.norm(diff2)
                if dist2 < self.CONTACT_DIST:
                    normal2 = diff2 / dist2 if dist2 > 1e-6 else normal
                    corrected = blade_pos + normal2 * self.CONTACT_DIST
                    puck['state'][0] = corrected[0]
                    puck['state'][1] = corrected[1]

        self._prev_blade_pos = blade_pos.copy()
    ...
    def update_and_publish(self):
        ...
        # Stick-pickup and stick-pushes-puck kinematics (see _apply_hockey_physics)
        self._apply_hockey_physics()
        ...
        # Update mobile robot plot (fast blit)
        self.__update_plot()

        # Update the arm simulator window (fast blit)
        self.arm_sim.update_plot()

        # Pump the Qt event loop once per tick so both windows actually repaint
        # on screen and stay responsive to mouse interaction. Since rclpy.spin()
        # blocks this thread, this is the only chance Qt gets to process events.
        self.figure.canvas.flush_events()
        self.arm_sim.fig.canvas.flush_events()
