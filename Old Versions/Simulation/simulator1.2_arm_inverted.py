...
class RobotArmSimulator:
    ...
    Joint 2 has a -90° offset (points downward when theta2 = 0).
    """
    def __init__(self, node, robot_id=10):
        ...
        # NOTE: previously had a -90° JOINT2_OFFSET purely for drawing convention.
        # The raw elbow angle from the law-of-cosines IK is already naturally within
        # [0, 180], so any nonzero offset here pushes valid solutions outside the
        # [THETA2_MIN, THETA2_MAX] range and causes the arm to clamp/freeze. Keep at 0.
    ...
    def target_arm_callback(self, msg: Point):
        ...
        Interpret msg.x and msg.z as absolute end-effector coordinates (x, y).
        Compute inverse kinematics with joint2 offset and update the arm.
        """
        target_x = msg.x
        target_y = msg.z   # map ROS Z to simulator Y (vertical)

        # Log reception for debugging
        self.node.get_logger().info(f"Arm command received: x={target_x:.3f}, y={target_y:.3f}")

        # Clamp target to workspace (safety)
        target_x = max(0.0, min(0.5, target_x))
        target_y = max(0.0, min(0.5, target_y))

        # Joint1 is at (BASE_WIDTH/2, BASE_HEIGHT)
        ...
        # Standard two-link IK without offset (elbow-up)
        cos_theta2 = (d*d - self.L1*self.L1 - self.L2*self.L2) / (2*self.L1*self.L2)
        cos_theta2 = max(-1.0, min(1.0, cos_theta2))
        theta2_solution = acos(cos_theta2)   # 0..pi, we'll clamp to 0..180°

        # Angle of the vector from J1 to target
        alpha = atan2(dy, dx)
        beta = atan2(self.L2*sin(theta2_solution), self.L1 + self.L2*cos(theta2_solution))
        theta1_solution = alpha - beta

        # Apply joint2 offset: the actual link2 angle is theta1 + theta2 + JOINT2_OFFSET
        # So commanded theta2 = (link2_angle) - theta1 - JOINT2_OFFSET
        # Here link2_angle = theta1 + theta2_solution (because we solved without offset)
        # Therefore commanded theta2 = theta2_solution - JOINT2_OFFSET
        theta2_solution = theta2_solution - self.JOINT2_OFFSET
    ...
    def compute_forward_kinematics(self):
        ...
        link2_angle = self.theta1 + self.theta2 + self.JOINT2_OFFSET
    ...
    def update_arm(self):
        ...
        if length > 1e-6:
            ...
            px = -uy
            py = ux
            half_open = 0.012 * self.gripper_opening
            p1 = (ee[0] + px * half_open, ee[1] + py * half_open)
            p2 = (ee[0] - px * half_open, ee[1] - py * half_open)
            self.gripper_lines[0].set_data([ee[0], p1[0]], [ee[1], p1[1]])
            self.gripper_lines[1].set_data([ee[0], p2[0]], [ee[1], p2[1]])
        else:
