...
class AlgorithmNode(Node):
    def __init__(self, robot_id):
        ...
        self.safe_distance = 0.50 # Safe distance for CBF (Collision Avoidance)
        self.gamma = 1.0          

        # Approximate linearization parameter
        self.kp = 0.85            # Navigation Aggressiveness
        self.l = 0.30            # Distance from robot center to virtual point P
        ...
        self.sub_stick = self.create_subscription(
            PoseStamped,
            '/vrpn_mocap/hockey_sticks_2/pose',   #ISSUE Change for Simulation and robot 
            self.stick_callback,
            best_effort_qos
        )
...
#----------------------------Callback & derivatives---------------------------------#
    ...
    def solve_clf_cbf_qp(self, ux_nom, uy_nom):
        ...
        for rid, x_obs, y_obs, distance in nearby_robots:
            ...
            cbf = (a1 * ux + a2 * uy + self.gamma * h)

            if cbf < 0.0:
                norm_sq = (a1*a1 + a2*a2)

                if norm_sq > 1e-6:
                    correction = (-cbf /norm_sq)
                    ux += (correction * a1)     # Debug: self.get_logger().info( f"CBF ACTIVE R{rid}")
                    uy += (correction * a2)
...
#-------------------------------Motion Controller's---------------------------------#
    ...
    def motion_controller(self):
        ...
        omega = (-ux * math.sin(theta) + uy * math.cos(theta)) / self.l

        omega += 0.8 * theta_error

        v = max(min(v, 0.5), -0.5)         # Bound linear velocity to [-0.5, 0.5] m/s
        omega = max(min(omega, 1.5), -1.5) # Bound angular velocity to [-1.5, 1.5] rad/s
    ...
    def grab_the_stick(self):
        ...
        if self.action_step == 0:
            self.arm_controller(x=0.20, z=0.05)
