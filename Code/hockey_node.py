...
class AlgorithmNode(Node):
    def __init__(self, robot_id):
        ...
        self.l = 0.20            # Distance from robot center to virtual point P 50
...
#----------------------------Callback & derivatives---------------------------------#
    def sub_pose_callback(self, msg):
        ...
        # Copilot Guidance
        # Filter heading at the SOURCE, not just the output -- robot_theta feeds cos/sin(theta) in every control law AND gets divided by the
        # small STEER_GAIN_L in the omega formula, so any raw mocap jitter in orientation gets amplified into a real angular velocity command
        # every tick. Smoothing v/omega afterward (apply_velocity) can only clean up so much if the controller keeps reacting to a flickering
        # heading in the first place. Averaged via (cos,sin) rather than the raw angle since naively blending angles breaks at the +-pi wrap.
        theta_alpha = 0.3
        self.filtered_cos = (self.filtered_cos + theta_alpha * (math.cos(theta_raw) - self.filtered_cos))
        self.filtered_sin = (self.filtered_sin + theta_alpha * (math.sin(theta_raw) - self.filtered_sin))
    ...
    def apply_velocity(self, v, omega):
        # Copilot Guidance
        # Low-pass filters v/omega against the previous tick's command before publishing. Each control method recomputes v/omega from scratch every tick (nominal control law + whatever the CBF
        # correction does that instant); when the CBF is continuously active for a long stretch (e.g. skimming past a nearby robot the whole
        # approach), that per-tick recomputation has no memory of the previous command, so any small overshoot in one direction gets
        # corrected, then overshoots the other way, tick after tick -- a zigzag. Blending with the previous command damps that out without changing the underlying control law.
...
#-------------------------------Motion Controller's---------------------------------#
    ...
    def go_to_checkpoint(self):
        ...
        # key geometry values that determine where the robot stops before grabbing.
    ...
    def go_to_goal_near(self):
        ...
        if distance < 0.10:
            ...
            self.action_step = 0
            self.get_logger().info("REACHED NEAR GOAL WAYPOINT")
            return
...
#---------------------------------Main()-----------------------------------------=--#
    def timer_callback(self):
        ...
        self.robot_traj_x.append(self.robot_x)
        self.robot_traj_y.append(self.robot_y)

        # Log stick
        if self.stick_x is not None:
            self.stick_traj_x.append(self.stick_x)
            self.stick_traj_y.append(self.stick_y)

        # Log puck
        if self.puck_x is not None:
            self.puck_traj_x.append(self.puck_x)
            self.puck_traj_y.append(self.puck_y)
...
def main(args=None):
    ...
    try:
        ...
        # Main trajectory CSV
        with open("robot_trajectory.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([
                "robot_x",
                "robot_y",
            ...
            n = min(
                len(node.robot_traj_x),
                len(node.stick_traj_x),
                len(node.puck_traj_x)
            )
            for i in range(n):
                writer.writerow([
                    node.robot_traj_x[i],
        ...
        with open("obstacles.csv", "w", newline="") as f:
            ...
            for rid, robot in node.other_robots.items():
                if robot["x"] is not None:
                    writer.writerow([rid, robot["x"], robot["y"]])
...
# Finally Working Version of the code with CBF and CLF implemented for collision avoidance and smooth navigation. The robot can now pick up the stick, navigate to the goal, and perform a swing shot while avoiding obstacles.